<!--
  AGENTS.md — spec proyek Rajaku Printing untuk agent coding apa pun.
  Isinya sama persis dengan CLAUDE.md; file ini ada supaya spec tetap terbaca
  saat proyek dikerjakan di luar Claude Code (Gemini Antigravity, Gemini CLI,
  Cursor, dsb) yang membaca AGENTS.md, bukan CLAUDE.md.
-->

# Rajaku Printing — Spec Proyek (untuk semua agent coding)

> **Baca ini dulu sebelum menulis kode apa pun di repo ini.**
> Dokumen ini bukan panduan gaya yang boleh ditawar. Sebagian besar aturannya
> lahir dari bug nyata yang sudah pernah terjadi di proyek ini, dan alasannya
...
- **Section 1–19** — arsitektur & fitur. Ini kontraknya; jangan mengarang
  status order, nama kolom, atau alur baru yang tidak ada di sini.
- **Section 22–23** — aturan coding Go. Wajib untuk semua kode backend.
- **Section 26** — Design Language untuk seluruh UI. Wajib untuk semua UI baru.
- **Section 28–32** — modul yang ditambahkan setelah spec awal. Kalau isinya
  bentrok dengan section lebih awal, **yang belakangan menang** (lihat blok
  Addendum di §24).

## Bagian yang khusus Claude Code

Dua section di bawah ini bicara soal peralatan Claude Code, bukan soal produk.
Kalau dikerjakan dengan agent lain, **isi aturannya tetap berlaku, cara
menjalankannya yang menyesuaikan**:

| Section | Isinya | Padanan di agent lain |
|---|---|---|
| §20 | Daftar "skill" berisi konvensi per topik | Jadikan file rujukan biasa, atau salin aturannya ke dalam prompt saat mengerjakan topik itu |
| §27 | Delegasi otomatis ke subagent (`.claude/agents/`) | Antigravity punya mekanisme agent/task sendiri; kalau tidak ada, kerjakan sendiri tapi **jangan lewati gate-nya** — review korektnes kode Go (§22) dan scan drift UI (§26) tetap wajib dilakukan sebelum sesuatu dianggap selesai |

Yang **tidak boleh** dibuang saat berpindah agent adalah gate-nya, bukan
mekanismenya: setiap kode Go tetap harus lolos pemeriksaan 4 aturan inti §22
(pemisahan layer, error dibungkus konteks, dilarang stub senyap, config
fail-fast), dan setiap file `.vue` tetap harus lolos pemeriksaan §26 (tanpa
kelas `rose`/`slate`/`red`, tanpa emoji, font hanya trio resmi).
...
- `docs/PANDUAN_PENGGUNAAN.md` — panduan untuk pengguna akhir, bukan untuk agent.
...
> **Catatan portabilitas:** "skill" di sini istilah Claude Code. Di agent lain
> (mis. Gemini Antigravity), perlakukan tabel di bawah sebagai **daftar topik
> yang aturannya wajib dibaca ulang dari section terkait** sebelum menyentuh
> area itu — bukan sebagai berkas yang harus dipasang. Isi aturannya ada di
> section yang dirujuk tiap baris, jadi tidak ada informasi yang hilang kalau
> mekanisme skill-nya tidak tersedia.
...
> **Catatan portabilitas:** section ini menjelaskan subagent Claude Code yang
> berkasnya ada di `.claude/agents/`. Agent lain tidak bisa membacanya.
>
> Yang **wajib tetap dijalankan** walau mekanismenya tidak ada — ini gate
> mutu, bukan kenyamanan:
>
> 1. **Setelah menulis/mengubah kode Go**, periksa ulang 4 aturan inti §22:
>    pemisahan layer handler → service → repository, error dibungkus konteks,
>    dilarang stub yang mengembalikan nilai palsu tanpa marker TODO, dan config
>    wajib fail-fast saat startup. Tambah: nama status harus persis §4, dan
>    respons harus memakai envelope §22.
> 2. **Setelah mengubah file `.vue` atau komponen UI**, scan pelanggaran §26:
>    kelas `rose-*`/`slate-*`/`red-*`/`gray-*`, emoji di UI, gradient
>    warna-warni, font di luar trio Fraunces/Inter/JetBrains Mono, `shadow-2xl`,
>    `rounded-full` pada tombol persegi, dan hex yang di-hardcode.
>
> Pembagian model di tabel di bawah (Opus/Sonnet/Haiku) adalah strategi biaya
> Claude Code. Nama modelnya ganti sesuai alat yang dipakai, tapi **jangan**
> ikut membuang pembagian kerjanya: implementasi dan pemeriksaan tetap dua
> langkah terpisah, karena yang menulis kode cenderung tidak melihat
> kesalahannya sendiri.

**Padanan model di Gemini Antigravity** (keputusan pemilik proyek,
8 September 2026):

| Jenis kerja | Di Claude Code | Di Antigravity |
|---|---|---|
| Review korektnes kode Go, keputusan arsitektur | Opus 5 | Gemini 3.1 Pro (High), atau Claude Opus 4.6 |
| Implementasi rutin modul Go / UI Nuxt | Sonnet 5 | **Gemini 3.8 Flash (High)** |
| Scan mekanis drift UI §26 (berbasis grep) | Haiku 4.5 | **Gemini 3.8 Flash (normal)** |

Mode penalaran-nya bagian dari keputusan, bukan detail bebas. Implementasi
memakai **High** karena aturan §22 & §26 harus ditimbang sambil menulis kode;
scan drift memakai mode **normal** karena kerjanya mencocokkan pola teks —
...
uang, atau migration — pakai model tier atas untuk **review**-nya walaupun
implementasinya memakai Flash.
